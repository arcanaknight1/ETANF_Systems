...
ETANFdata$Year <- as.factor(ETANFdata$Year)
...
```{r}
### Data Visualization
p1 <- ggplot(ETANFdata,aes(ETANF)) + geom_bar() + xlab("ETANF") + scale_x_continuous(limits = c(0, 350)) #limited scale to exclude outliers in Pit & Philly
p1
```
...
```{r}
### Regression model
etanf.model <- glm(ETANF~Year + PA_Region + SNAP_July + Medicaid + CHIP + LFPR + Med_Income + ED_Attain_HSandAbove + Ed_Attain_BAandAbove + Pov_Rate + Population, data = ETANFdata, family = "poisson")
